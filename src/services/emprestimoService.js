...
/**
 * Cria um empréstimo.
 * Body esperado: { usuarioId, exemplarId, diasPrazo? }
 * Cria também o item_emprestimo vinculado.
 */
...
async function renovar(id, { diasAdicionais = 7 }) {
  ...
  return prisma.emprestimo.update({
    where: { emprestimo_id: Number(id) },
    data: {
      emprestimo_data_prevista_devolucao: addDays(
        emp.emprestimo_data_prevista_devolucao,
        diasAdicionais
      ),
    },
  });
...
async function deletar(id) {
  ...
  return prisma.emprestimo.delete({ where: { emprestimo_id: Number(id) } });
}
